-- Patch 5.1 — Filter event-exclusive cards out of NON-event chests.
--
-- We extend open_chest's item-pick query so that event-exclusive items only
-- drop from chests that belong to the same event. Once the event ends and
-- end_event() flips is_event_exclusive to FALSE, the card becomes a normal
-- pool drop automatically.
    ...
    LOOP
      SELECT si.id, si.name, si.rarity
       ...
       WHERE si.rarity = v_rarity
         ...
         -- Patch 5.1: event-exclusive items only drop from chests of the
         -- SAME event. After the event ends, is_event_exclusive flips off.
         AND (
