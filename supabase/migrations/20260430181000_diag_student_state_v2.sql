  ...
  SELECT jsonb_build_object(
    ...
    'current_chars',
      COALESCE((
        ...
        WHERE  c.user_id = (SELECT user_id FROM public.students WHERE id = p_student_id)
      ), '[]'::jsonb),
    'orphan_chars_with_progress',
      COALESCE((
        ...
        FROM   public.characters c
        WHERE  c.user_id IS DISTINCT FROM (SELECT user_id FROM public.students WHERE id = p_student_id)
          AND  (
                c.user_id IS NULL
