...
export function useTeacherAnalytics(teacherId: string | undefined) {
  const [filters, setFilters] = useState<AnalyticsFilters>({ period: 30 });
  ...
  const [riskScores, setRiskScores] = useState<{ student_id: string; student_name: string; level: number; days_since_last_login: number; logins_last_14_days: number; missions_last_14_days: number; streak_current: number; risk_score: number }[] | null>(null);
  ...
  const loadAll = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const [overviewRes, heatmapRes, dailyRes, riskRes] = await Promise.all([
        ...
        supabase.rpc("get_daily_activity", { p_teacher_id: teacherId, p_days: filters.period }),
