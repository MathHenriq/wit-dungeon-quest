/**
 * Catálogo oficial: as cartas da loja do WIT Dungeon 1 traduzidas para o TCG.
 *
 * Cada entrada guarda o nome que o item tinha na loja (`shop`), para migrar o
 * inventário dos alunos e reaproveitar a arte. O texto da carta NÃO é escrito
 * aqui: sai de `describeCard`, a partir dos efeitos.
 *
 * Faixa de poder (vida 150, 1 Ataque por turno):
 *   Comum 10–14 · Incomum 12–18 · Rara 16–20 · Épica 18–30
 *   Lendária 24–40 com custo · Mítica 30–70 com custo pesado
 *   Desconhecida: muda o jogo, e cobra caro por isso.
 */

import type { CardDef, Element, Rarity } from '../types';

interface Entry {
  /** Nome do item na loja antiga (chave para migração e arte). */
  shop: string;
  card: CardDef;
}
...
function make(type: CardDef['type']) {
...
const atk = make('attack');
const des = make('challenger');
const eqp = make('equipment');
const arm = make('trap');
const cmp = make('field');
...
const ARMADURAS: Entry[] = [
  eqp('Manto da Aurora (Edição Premium)', 'Manto da Aurora', 'Ice', 'epic', null, {
    slot: 'armor',
    passives: [
  ...
  des('Modo Sábio dos Seis Caminhos', 'Modo Sábio dos Seis Caminhos', 'Flying', 'mythic', 'Naruto', {
...
const ARMAMENTOS_EPICOS: Entry[] = [
  atk('Baruka’s Dagger', 'Adaga de Baruka', 'Ice', 'epic', 'Solo Leveling', {
    damage: 20,
    effects: [{ kind: 'status', status: 'bleed', value: 4, turns: 3 }],
  }),
  atk('Black Divider', 'Black Divider', 'Dark', 'epic', 'Black Clover', {
    damage: 22,
    effects: [{ kind: 'destroy', what: 'trap' }],
  }),
  des('Chain Jail', 'Prisão de Correntes', 'Steel', 'epic', 'Hunter x Hunter', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'lock', cardType: 'challenger', turns: 2 }, { kind: 'draw', count: 1 }],
  }),
  des('Cursed Speech', 'Fala Amaldiçoada', 'Dark', 'epic', 'Jujutsu Kaisen', {
    cost: [{ kind: 'payLife', amount: 10 }],
    effects: [{ kind: 'status', status: 'freeze', turns: 1 }, { kind: 'discardRandom', count: 1 }],
  }),
  atk('Fire Dragon Roar', 'Rugido do Dragão de Fogo', 'Fire', 'epic', 'Fairy Tail', {
    damage: 22,
  ...
  des('Kakuja Form', 'Forma Kakuja', 'Dark', 'epic', 'Tokyo Ghoul', {
    cost: [{ kind: 'payLife', amount: 10 }],
    effects: [{ kind: 'addModifier', spec: { match: { type: 'attack', element: 'Dark' }, add: 10, uses: 2, label: 'Kakuja' } }],
  }),
  atk('Kasaka’s Venom Fang', 'Presa Venenosa de Kasaka', 'Poison', 'epic', 'Solo Leveling', {
    damage: 14,
  ...
  eqp('Orb of Avarice', 'Orbe da Avareza', 'Dark', 'epic', 'Solo Leveling', {
    slot: 'weapon',
    cost: [{ kind: 'discard', count: 1 }],
    passives: [{ kind: 'attackBonus', match: { type: 'attack' }, add: 6, label: 'Orbe da Avareza' }],
  }),
  atk('Playful Cloud', 'Nuvem Brincalhona', 'Fighting', 'epic', 'Jujutsu Kaisen', {
    damage: 18,
  ...
  atk('Punho de Ferro', 'Punho de Ferro', 'Fighting', 'epic', 'Vinland Saga', {
    damage: 14,
  ...
  des('Ten Shadows', 'Dez Sombras', 'Dark', 'epic', 'Jujutsu Kaisen', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'aura', turns: 3, label: 'Shikigami', effects: [{ kind: 'damage', amount: 6 }] }],
  }),
  atk('Toque de Decadência', 'Toque de Decadência', 'Dark', 'epic', 'My Hero Academia', {
    damage: 16,
...
const ARMAMENTOS_LENDARIOS: Entry[] = [
  atk('Black Whip', 'Chicote Negro', 'Fighting', 'legendary', 'My Hero Academia', {
    damage: 28,
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'lock', cardType: 'equipment', turns: 2 }],
  }),
  des('Bungee Gum', 'Chiclete Elástico', 'Ghost', 'legendary', 'Hunter x Hunter', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [
  ...
  eqp('Chastiefol', 'Chastiefol', 'Grass', 'legendary', 'Seven Deadly Sins', {
    slot: 'weapon',
    cost: [{ kind: 'discard', count: 1 }],
    passives: [
  ...
  atk('Death Scythe', 'Foice da Morte', 'Ghost', 'legendary', 'Soul Eater', {
    damage: 25,
    cost: [{ kind: 'payLife', amount: 10 }],
    effects: [{
      kind: 'conditional',
  ...
  atk('Demon Sword Ragnarok', 'Espada Demoníaca Ragnarok', 'Dark', 'legendary', 'Soul Eater', {
    damage: 24,
    cost: [{ kind: 'discard', count: 1 }],
  ...
  atk('Espírito da Besta', 'Espírito da Besta', 'Fighting', 'legendary', 'Demon Slayer', {
    damage: 20,
    cost: [{ kind: 'payLife', amount: 10 }],
    effects: [{ kind: 'addModifier', spec: { match: { type: 'attack', element: 'Fighting' }, add: 10, uses: 2, label: 'Respiração da Besta' } }],
  }),
  atk('Gáe Bolg', 'Gáe Bolg', 'Steel', 'legendary', 'Fate/stay night', {
    damage: 30,
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'pierce' }],
  }),
  atk('Gideon', 'Gideon', 'Ground', 'legendary', 'Seven Deadly Sins', {
    damage: 28,
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'destroy', what: 'armor' }],
  }),
  cmp('Hyorinmaru', 'Hyorinmaru', 'Ice', 'legendary', 'Bleach', {
    passives: [
  ...
  des('Lostvayne', 'Lostvayne', 'Dark', 'legendary', 'Seven Deadly Sins', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'aura', turns: 3, label: 'Clones de Lostvayne', effects: [{ kind: 'damage', amount: 10 }] }],
  }),
  atk('Mjolnir', 'Mjölnir', 'Electric', 'legendary', 'Record of Ragnarok', {
    damage: 26,
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{
      kind: 'conditional',
  ...
  atk('Murasame', 'Murasame', 'Poison', 'legendary', 'Akame ga Kill', {
    damage: 16,
    cost: [{ kind: 'discard', count: 1 }],
    effects: [
      {
  ...
  atk('Rhitta', 'Rhitta', 'Fire', 'legendary', 'Seven Deadly Sins', {
    damage: 20,
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'bonus', add: { per: 'round', each: 4, max: 32 }, label: 'Sol do Meio-Dia' }],
  }),
  atk('Senbonzakura', 'Senbonzakura', 'Grass', 'legendary', 'Bleach', {
    damage: 16,
    cost: [{ kind: 'discard', count: 1 }],
  ...
  eqp('Volundr', 'Völundr', 'Fire', 'legendary', 'Fate', {
    slot: 'weapon',
    cost: [{ kind: 'discard', count: 1 }],
    passives: [
  ...
  atk('Z Sword', 'Espada Z', 'Steel', 'legendary', 'Dragon Ball', {
    damage: 24,
    cost: [{ kind: 'discard', count: 1 }],
...
const ARMAMENTOS_MITICOS: Entry[] = [
  eqp('Avalon', 'Avalon', 'Steel', 'mythic', 'Fate', {
    slot: 'armor',
    cost: [{ kind: 'discard', count: 2 }],
    passives: [
  ...
  atk('Dragon of the Darkness Flame', 'Dragão das Chamas Negras', 'Dark', 'mythic', 'Yu Yu Hakusho', {
    damage: 35,
    cost: [{ kind: 'payLife', amount: 20 }],
    effects: [{ kind: 'status', status: 'burn', value: 8, turns: 3 }],
  }),
  atk('Excalibur', 'Excalibur', 'Steel', 'mythic', 'Fate', {
    damage: 40,
    cost: [{ kind: 'discard', count: 2 }],
    effects: [{
      kind: 'conditional',
  ...
  atk('Kagune Liberado', 'Kagune Liberado', 'Dark', 'mythic', 'Tokyo Ghoul', {
    damage: 30,
  ...
  des('Limitless', 'Ilimitado', 'Ghost', 'mythic', 'Jujutsu Kaisen', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [{ kind: 'shield', count: 3 }, { kind: 'damage', amount: 15 }],
  }),
  atk('Spirit Gun', 'Reigan', 'Ghost', 'mythic', 'Yu Yu Hakusho', {
    damage: 20,
    cost: [{ kind: 'discard', count: 1 }],
...
const ARMAMENTOS_DESCONHECIDOS: Entry[] = [
  des('Coordenadas do Titã Fundador', 'Coordenada', 'Ground', 'unknown', 'Attack on Titan', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [
  ...
  atk('Enuma Elish', 'Enuma Elish', 'Flying', 'unknown', 'Fate', {
    damage: 50,
    cost: [{ kind: 'discard', count: 3 }],
    effects: [{ kind: 'pierce' }],
  }),
  des('Kamish', 'Kamish, o Dragão', 'Fire', 'unknown', 'Solo Leveling', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [
  ...
  atk('Kamish’s Wrath', 'Fúria de Kamish', 'Fire', 'unknown', 'Solo Leveling', {
    damage: 20,
  ...
  des('Requiem', 'King Crimson', 'Dark', 'unknown', 'JoJo', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [
...
const COLECAO: Entry[] = [
  ...
  eqp('Coroa do Crepúsculo (Premium)', 'Coroa do Crepúsculo', 'Dark', 'legendary', null, {
    slot: 'armor',
    passives: [
  ...
  des('Dragon Balls', 'Esferas do Dragão', 'Flying', 'mythic', 'Dragon Ball', {
    cost: [{ kind: 'banish', count: 5 }],
    effects: [{ kind: 'heal', amount: 50 }, { kind: 'draw', count: 2 }],
  }),
  des('Requiem Arrow', 'Flecha Requiem', 'Ghost', 'mythic', 'JoJo', {
...
const HABILIDADES: Entry[] = [
  ...
  // épicas
  atk('Anti-Magic Slash', 'Corte Antimagia', 'Dark', 'epic', 'Black Clover', {
    damage: 20,
  ...
  des('Caixão das Estrelas', 'Caixão das Estrelas', 'Ghost', 'epic', 'Frieren', {
    effects: [{ kind: 'damage', amount: 15 }, { kind: 'purge', what: 'modifiers', target: 'opponent' }],
  }),
  atk('Cruel Sun', 'Sol Cruel', 'Fire', 'epic', 'Seven Deadly Sins', {
    damage: 22,
    effects: [{ kind: 'status', status: 'burn', value: 4, turns: 2 }],
  }),
  atk('Dimension Slash', 'Corte Dimensional', 'Dark', 'epic', 'Black Clover', {
    damage: 18,
    effects: [{ kind: 'pierce' }],
  }),
  des('Dragon Force', 'Dragon Force', 'Fire', 'epic', 'Fairy Tail', {
    cost: [{ kind: 'payLife', amount: 10 }],
    effects: [{ kind: 'addModifier', spec: { match: { type: 'attack', element: 'Fire' }, add: 8, uses: 3, label: 'Dragon Force' } }],
  }),
  des('Equivalent Exchange', 'Troca Equivalente', 'Ground', 'epic', 'Fullmetal Alchemist', {
    cost: [{ kind: 'discard', count: 1 }],
  ...
  atk('Flame Alchemy', 'Alquimia das Chamas', 'Fire', 'epic', 'Fullmetal Alchemist', {
    damage: 18,
  ...
  des('Godspeed', 'Velocidade Divina', 'Electric', 'epic', 'Hunter x Hunter', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [
      { kind: 'shield', count: 1 },
  ...
  atk('Magia de Roswaal', 'Magia de Roswaal', 'Fire', 'epic', 'Re:Zero', {
    damage: 18,
    effects: [{
      kind: 'conditional',
      if: { kind: 'lifeAtLeast', amount: 120 },
  ...
  des('Ope Ope no Mi (ROOM)', 'ROOM', 'Ghost', 'epic', 'One Piece', {
    effects: [{ kind: 'destroy', what: 'trap' }, { kind: 'destroy', what: 'trap' }, { kind: 'draw', count: 1 }],
  }),
  eqp('Orbe Cromático (Edição Premium)', 'Orbe Cromático', 'Electric', 'epic', null, {
    slot: 'weapon',
  ...
  des('Shadow Extraction', 'Extração de Sombras', 'Dark', 'epic', 'Solo Leveling', {
    cost: [{ kind: 'payLife', amount: 10 }],
  ...
  atk('Thunderclap and Flash', 'Primeira Forma: Relâmpago', 'Electric', 'epic', 'Demon Slayer', {
    damage: 24,
    cost: [{ kind: 'mill', count: 1 }],
  }),
  ...
  des('Edo Tensei', 'Edo Tensei', 'Ghost', 'legendary', 'Naruto', {
    cost: [{ kind: 'banish', count: 2 }],
    effects: [{ kind: 'aura', turns: 4, label: 'Reanimados', effects: [{ kind: 'damage', amount: 8 }] }],
  }),
  des('Jaula de Espinhos', 'Jaula de Espinhos', 'Grass', 'legendary', 'Frieren', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [
      { kind: 'lock', cardType: 'attack', turns: 1 },
  ...
  des('O Avatar', 'Estado Avatar', 'Flying', 'legendary', 'Avatar', {
    cost: [{ kind: 'discard', count: 1 }],
    effects: [{ kind: 'addModifier', spec: { match: { type: 'attack' }, add: 6, uses: 4, label: 'Estado Avatar' } }],
  }),

  // míticas
  atk('Contrato com o Diabo', 'Contrato com o Diabo', 'Dark', 'mythic', 'Chainsaw Man', {
    damage: 70,
  ...
  des('Haki do Rei Conquistador', 'Haki do Rei', 'Fighting', 'mythic', 'One Piece', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [
  ...
  atk('Zoltraak', 'Zoltraak', 'Dark', 'mythic', 'Frieren', {
    damage: 30,
    cost: [{ kind: 'discard', count: 1 }],
  ...
  // desconhecidas
  des('Geass Eye', 'Olho do Geass', 'Dark', 'unknown', 'Code Geass', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [
  ...
  des('Ultra Instinct', 'Instinto Superior', 'Flying', 'unknown', 'Dragon Ball', {
    cost: [{ kind: 'discard', count: 2 }],
    effects: [
      { kind: 'shield', count: 1 },
  ...
  // token
  eqp('Selo do Vazio (Premium)', 'Selo do Vazio', 'Ghost', 'legendary', null, {
    slot: 'armor',
    passives: [
...
export const CARD_ID_BY_SHOP_NAME: ReadonlyMap<string, string> = new Map(ENTRIES.map(e => [e.shop, e.card.id]));
